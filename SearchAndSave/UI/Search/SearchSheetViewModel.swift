    ...
    actor Reactor {
        ...
        @MainActor func applyBookmark(state: inout State) async {
            ...
            let withBookmarks = state.results.map { bookmark in
                ...
                if bookmarks.filter({ $0.thumbnail == bookmark.thumbnail }).first != nil {
            ...
            Future<SearchApi.ImageResponseDTO, Error> { promise in
                Task { [weak self] in
                    ...
                    do {
                        ...
                        Logger.log("fetch Image page = \(page), count = \(searched.documents.count)")
            ...
            Future<SearchApi.VClipResponseDTO, Error> { promise in
                Task { [weak self] in
                    ...
                    do {
                        ...
                        Logger.log("fetch VClip page = \(page), count = \(searched.documents.count)")
                        promise(.success(searched))
                    } catch {
                        Logger.log("error = \(error)")
                        promise(.failure(error))
